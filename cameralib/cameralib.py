...
from cameralib.decorators import camera_transform, point_transform
...
class Camera:
    ...
    def rotate(self, yaw=0, pitch=0, roll=0):
        ...
        camera_rotation = Rotation.from_euler('YXZ', [yaw, pitch, roll]).as_matrix()
    ...
    def get_pitch_roll(self):
        yaw, pitch, roll = Rotation.from_matrix(self.R).as_euler('YXZ')
        return pitch, roll
        ...
        if alpha_balance is not None and self.has_distortion():
            ...
            if len(self.distortion_coeffs) == 4:
                ...
                self.intrinsic_matrix = cv2.getOptimalNewCameraMatrix(
                    ...
                    new_imsize, centerPrincipalPoint=center_principal_point)[0]
    ...
    @camera_transform
    def turn_towards(self, target_image_point=None, target_world_point=None, target_cam_point=None):
...
def reproject_image_points(points, old_camera, new_camera):
    ...
    if not old_camera.has_distortion() and not new_camera.has_distortion() and points.ndim == 2:
        return reproject_image_points_fast(points, old_camera, new_camera)

    if not np.allclose(old_camera.t, new_camera.t):
        raise Exception('The optical centers of the cameras differ, warping is ambiguous!')
